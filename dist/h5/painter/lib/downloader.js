 ...
 */
import Taro, { getStorage as _getStorage, getFileInfo as _getFileInfo, getSavedFileInfo as _getSavedFileInfo, base64ToArrayBuffer as _base64ToArrayBuffer, getFileSystemManager as _getFileSystemManager, downloadFile as _downloadFile, setStorage as _setStorage, saveFile as _saveFile, removeStorage as _removeStorage, getSavedFileList as _getSavedFileList, removeSavedFile as _removeSavedFile } from "@tarojs/taro-h5";
import { isValidUrl, isDataUrl, isOnlineUrl } from './util';
...
function getFileName(url) {
  if (isDataUrl(url)) {
    ...
    const fileName = `${hex_sha1(bodyData)}.${format}`;
    return fileName;
  } else {
    return url;
  }
...
function transformBase64File(base64data, lru) {
  return new Promise((resolve, reject) => {
    ...
    const buffer = _base64ToArrayBuffer(bodyData.replace(/[\r\n]/g, ''));
    _getFileSystemManager().writeFile({
      filePath: path,
      data: buffer,
      encoding: 'binary',
      success() {
